package EPrints::Plugin::Screen::REF2021::Report::REF1c;
...
sub users
{
		...
		[
                	{ meta_fields => [ "ref2021_uoa" ], value => join( " ", @uoa_ids ), merge => 'ANY' },
                	{ meta_fields => [ "ref_category" ], value => "C" },
        ], keep_cache => 1);
...
sub ajax_user
{
	...
	->map(sub {
		...
		# don't display user in REF20211c report if that user isn't a Cat C staff:
		my $cat = $user->value( 'ref_category' );
		return if( !defined $cat || $cat ne 'C' );
...
sub render_user
{
	...
	$chunk->appendChild( $circ->render_citation( 'ref1c' ) );
